...
In this chapter, we will introduce the **{lubridate}** package that is designed
to make it easier to work with dates and times in R.
...
is specify the order of the year `y`, month `m`, and the day `d` and {lubridate}
will automatically figure out the format.
...
Date vs date-time objects that are created using {lubridate}. By default, dates
will be created as `Date` objects without the time component. You can force the
creation of a date-time object by including the timezone `tz` argument.
