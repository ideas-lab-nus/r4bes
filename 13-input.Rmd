...
In this chapter, we will use the **{eplusr}** package to extract various inputs
in an EnergyPlus model, and through this, learn how the inputs in EnergyPlus are
structured.
...
{eplusr} lets you to query model elements by names using the `$` operator, just
like a normal named list in R. For instance, you can access the objects defined
using the `Material:NoMass` class with `$Material:NoMass`. Since class, object,
and field names in EnergyPlus often violets the variable naming restrictions in
R (e.g., containing special characters such as `:`, white spaces, etc.), the
identifiers need to be placed within a set of single back quotes `` ` `` to tell
R that this is a variable name.
...
\@ref(fig:input-structure). The required fields are indicated by `eplusr` using
an asterix `*`. Using the `Material:NoMass` class as an example (Figure
