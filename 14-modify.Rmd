...
In this chapter, we will use the **{eplusr}** package to query and modify
various inputs in an EnergyPlus model. We will also combine what you have
learned about data manipulation (with the **{tidyverse}** package) to make it
easier to work with the simulation inputs.
...
-   `Roughness` is a required field of type `choice` and the possible choices
    are `VeryRough`, `Rough`, `MediumRou.gh`, `MediumSmooth`, `Smooth`, and
    `VerySmooth`.
